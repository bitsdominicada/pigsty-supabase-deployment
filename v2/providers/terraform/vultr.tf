...
locals {
  ...
  meta_plan = "vc2-2c-4gb"     # infra/meta: shared 2 vCPU, 4GB RAM
  db_plan   = "vhp-4c-8gb-amd" # pg data nodes: dedicated 4 vCPU, 8GB RAM
...
# VPC
#===========================================================#
resource "vultr_vpc" "pigsty_vpc" {
  description    = "Pigsty VPC Network"
  region         = var.region
  v4_subnet      = "10.10.10.0"
  v4_subnet_mask = 24
}
...
resource "vultr_instance" "pg_data_1" {
  label             = "pg-data-1"
  hostname          = "pg-data-1"
  region            = var.region
  plan              = local.db_plan
  ...
  tags = ["pigsty", "pgsql", "pg-data-1"]
}

resource "vultr_instance" "pg_data_2" {
  label             = "pg-data-2"
  hostname          = "pg-data-2"
  region            = var.region
  plan              = local.db_plan
...
output "db1_ip_ephemeral" {
  description = "Ephemeral public IP of pg-data-1"
  value       = vultr_instance.pg_data_1.main_ip
}

output "db1_private_ip" {
  description = "Private IP of pg-data-1"
  value       = vultr_instance.pg_data_1.internal_ip
}

output "db2_ip_ephemeral" {
  description = "Ephemeral public IP of pg-data-2"
  value       = vultr_instance.pg_data_2.main_ip
}

output "db2_private_ip" {
  description = "Private IP of pg-data-2"
  value       = vultr_instance.pg_data_2.internal_ip
}

output "cluster_private_ips" {
  description = "Private IP list for 3-node cluster"
  value = {
    meta = vultr_instance.pg-meta.internal_ip
    db1  = vultr_instance.pg_data_1.internal_ip
    db2  = vultr_instance.pg_data_2.internal_ip
  }
}
